...
/**
 * Ajustes por pestanas.
 *
 * Era una sola columna de scroll con seis tarjetas seguidas: para cambiar la
 * hora del aviso habia que pasar por delante de las credenciales de Google.
 * Agrupado, cada cosa esta donde uno la busca.
 *
 * El formulario sigue siendo uno solo por debajo: las pestanas solo deciden
 * que se ve. Por eso el boton de guardar aparece en cualquiera de ellas en
 * cuanto hay algo sin guardar, y no se pierde nada al cambiar de pestana.
 */

const PESTANAS = [
  { id: 'cuenta', etiqueta: 'Cuenta' },
  { id: 'cerebro', etiqueta: 'Cerebro' },
...
export default function Ajustes(): JSX.Element {
  ...
  /** Hay algo escrito que aun no se ha guardado. */
  const hayCambios =
    settings !== null &&
    (clientId.trim() !== settings.googleClientId ||
      provider !== settings.llmProvider ||
      model.trim() !== settings.geminiModel ||
  ...
  const save = async (): Promise<void> => {
    ...
    if (result.ok) {
      setClientSecret('')
      setGeminiKey('')
      avisos.mostrar('Ajustes guardados.')
  ...
  const importarJson = async (): Promise<void> => {
    ...
    if (result.ok) {
      ...
      if (result.data) {
        avisos.mostrar('Credenciales importadas.')
        settingsState.reload()
      }
    } else {
      avisos.error(result.error)
  ...
  const volverAIncluidas = async (): Promise<void> => {
    ...
    if (updated.ok) {
      setClientId('')
      setClientSecret('')
      avisos.mostrar('Se usaran las credenciales incluidas.')
      settingsState.reload()
    }
    setBusy(false)
  }
  ...
  const exportarDatos = async (): Promise<void> => {
    ...
    else if (r.data) avisos.mostrar(`Copia guardada en ${r.data.ruta}`)
    setBusy(false)
  }
  ...
  const importarDatos = async (): Promise<void> => {
    ...
    if (!r.ok) avisos.error(r.error)
      ...
      avisos.mostrar(
        `Restaurados ${r.data.ficheros} fichero(s). Cierra y vuelve a abrir JARVIS para verlos.`
      )
    }
    setBusy(false)
  }
  ...
  const replayIntro = async (): Promise<void> => {
    ...
    avisos.mostrar('La veras la proxima vez que abras JARVIS.')
  }
  ...
  const connect = async (): Promise<void> => {
    ...
    if (result.ok) {
      ...
      avisos.mostrar(`Conectado como ${result.data.email ?? 'tu cuenta'}.`)
    } else {
      avisos.error(result.error)
    }
    setBusy(false)
  ...
  return (
    <>
        ...
        <div className="card">
          ...
          {!settings?.listoParaConectar && (
            <p className="hint" style={{ marginTop: 10 }}>
              Esta copia de JARVIS no trae credenciales incluidas. Configura las tuyas aqui abajo.
            </p>
          )}
          ...
          {provider === 'ollama' ? (
            <>
              ...
              <div className="field">
                ...
                <p className="hint" style={{ marginTop: 8 }}>
                  El modelo debe soportar herramientas (tool calling). `llama3.1:8b` y `qwen2.5:7b`
                  funcionan bien.
                </p>
              </div>
            ...
            <>
              ...
              <div className="field">
                ...
                <p className="hint">Los modelos Flash son los que tienen cuota gratuita amplia.</p>
              </div>
            </>
          )}
        </div>
      ...
      {pestana === 'apariencia' && (
        <div className="card">
          ...
          <div className="list-item">
            ...
            <button onClick={() => void toggleGlass()} aria-pressed={settings?.glassEnabled ?? true}>
        ...
        <div className="card">
          ...
          <div className="list-item">
            <div>
              ...
              <div className="meta">
                Tus tareas, tus examenes, las reglas de carpetas y la conversacion, en un fichero.
              </div>
            </div>
      ...
      {/* Solo aparece cuando hay algo que guardar. Asi no hay un boton
          permanente que no hace nada en las pestanas que se aplican solas, y a
          la vez nada se pierde al cambiar de pestana: el formulario es uno. */}
      {hayCambios && (
        <div className="guardar-barra">
          <span className="meta">Tienes cambios sin guardar.</span>
